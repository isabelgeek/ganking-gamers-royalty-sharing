...
const sharedRoyaltyBehavior = require('ganking-tokens/test/sharedRoyaltyToken.behavior');
const utilFactory = require('ganking-tokens/test/erc721UtilFactory');

contract('GankingToken', accounts => {
  let token;
  let tokenId;
  let util;
   ...
   *
   * @returns {Promise<{GankingToken}>} - A Ganking Token instance
   */
  const mintToken = async () => {
    const id = web3.utils.asciiToHex('token id');
    const instance = await BootlegToken.new('Ganking', 'GANKING', 5);
