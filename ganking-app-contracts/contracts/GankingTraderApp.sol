...
import "./GankingToken.sol";


contract GankingTraderApp {
    // The ganking token contract
    GankingToken public gankingToken;
    // Only trading one token for this dApp, this is the ID for the token
    uint256 public gankingTokenId;
    ...
    /**
     * @notice Create a new Ganking Token trading app
     *
     * @param deployedAddress address Deployed address of the GankingToken contract
     * @param tokenId uint256 TokenID to trade with this app
     */
    constructor(address deployedAddress, uint256 tokenId) public {
        gankingToken = GankingToken(deployedAddress);
        gankingTokenId = tokenId;
    }

    /**
     * @notice Provides a way to become a franchisor a Ganking token for a fee
    ...
    function purchase() public payable {
        ...
        address owner = gankingToken.ownerOf(gankingTokenId);
        // Do the token transfer
        gankingToken.safeTransferFrom.value(msg.value)(
            owner,
            newOwner,
            gankingTokenId,
            ""
    ...
    function setTokenPrice(uint256 newPrice) public {
        address currentOwner = gankingToken.ownerOf(gankingTokenId);
    ...
    function getOwner() public view returns (address) {
        return gankingToken.ownerOf(gankingTokenId);
    }
    ...
    function getBalance() public view returns (uint256) {
        return gankingToken.paymentBalanceOf(msg.sender, gankingTokenId);
    }

    /**
     * @notice Use to widthraws eth from the payments you recieved during franchising
     * Emits a PaymentWithdrawn event
     * @dev Calls the withdrawPayment function on the GankingToken
     */
    function withdraw() public {
        gankingToken.withdrawPayment(msg.sender, gankingTokenId);
    ...
    function getTokenURI() public view returns (string memory) {
        return gankingToken.tokenURI(gankingTokenId);
    }
    ...
    {
        return
            gankingToken.isTokenFranchisor(possibleFranchisor, gankingTokenId);
    }
}
